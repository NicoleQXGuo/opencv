...
def model_from_checkpoint_path(checkpoints_path):
    ...
    print("loading _config.json Running time: " + str(time() - start_time))
    latest_weights = find_latest_checkpoint(checkpoints_path)
    print("find_latest_checkpoint Running time: " + str(time() - start_time))
    assert (latest_weights is not None), "Checkpoint not found."
    model = model_from_name[model_config['model_class']](
        model_config['n_classes'], input_height=model_config['input_height'],
        input_width=model_config['input_width'])
    print("loaded weights ", latest_weights)
    print("latest_weights Running time: " + str(time() - start_time))
...
def predict(model=None, inp=None, out_fname=None,
    ...
    print("model_from_checkpoint_path Running time: " + str(time() - start_time))
    ...
    end_time = time()
    print("after model.predict Running time: " + str(end_time - start_time))
    start_time = time()
    pr = model.predict(np.array([x]))[0]
    end_time = time()
    print("model.predict Running time: " + str(end_time - start_time))
